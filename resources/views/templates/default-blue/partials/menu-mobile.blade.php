{{-- Recursive mobile menu partial --}}
{{-- Usage: @include(\App\Helpers\Template::view('partials.menu-mobile'), ['items' => $collection, 'depth' => 0]) --}}
...
@foreach($items as $item)
        ...
        <div x-data="{ expanded: false }">
            <div class="flex items-center">
                ...
                <a href="{{ $item->url }}" target="{{ $item->target }}" @click="mobileOpen=false"
                   class="flex-1 block px-3 py-2 text-sm text-gray-600 hover:bg-blue-50 rounded-lg {{ $depth > 0 ? 'pl-' . (3 + $depth * 4) : '' }}">
                    {{ $item->title }}
                </a>
                @else
                <span class="flex-1 block px-3 py-2 text-sm text-gray-500 {{ $depth > 0 ? 'pl-' . (3 + $depth * 4) : '' }}">
                    {{ $item->title }}
                </span>
                @endif
    ...
    @else
        {{-- Simple link --}}
        <a href="{{ $item->url ?: '#' }}" target="{{ $item->target }}" @click="mobileOpen=false"
           class="block px-3 py-2 text-sm text-gray-600 hover:bg-blue-50 rounded-lg" style="{{ $depth > 0 ? 'padding-left: ' . (0.75 + $depth * 1) . 'rem;' : '' }}">
            {{ $item->title }}
        </a>
    @endif
@endforeach
