        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @php
                $activePaymentMethods = collect($activeConference->payment_methods ?? [])
                    ...
                    ->values();
            @endphp
            @if($activePaymentMethods->isNotEmpty() || $activeConference->payment_bank_name || $activeConference->payment_bank_account || $activeConference->payment_contact_phone)
            <div class="mt-12 bg-white rounded-2xl shadow-sm border p-6 md:p-8">
                ...
                @if($activePaymentMethods->isNotEmpty())
                    ...
                    <div class="mt-6 bg-green-50 rounded-xl p-5 border border-green-200 flex items-center gap-2 justify-center">
                        ...
                        <span class="text-sm text-gray-600">{{ __('welcome.pricing.kontak_pembayaran') }}:</span>
                        <span class="text-sm font-bold text-gray-800">{{ $activeConference->payment_contact_phone }}</span>
                    </div>
                    @endif
                @else
                    {{-- Legacy single-method fields (no payment methods added yet) --}}
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
