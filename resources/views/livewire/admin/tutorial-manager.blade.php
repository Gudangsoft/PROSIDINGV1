...
<div class="max-w-5xl mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
        <h2 class="text-base font-semibold text-gray-800 mb-5">
            {{ $editingId ? 'Edit Tutorial' : 'Tambah Tutorial Baru' }}
        </h2>
        <div class="space-y-4">
            <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    @error('pdfFile')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50/50 transition">
                    <td class="px-4 py-3 text-gray-400 font-mono text-xs">{{ $tutorial->sort_order }}</td>
                    <td class="px-4 py-3">
                        <p class="font-medium text-gray-800">{{ $tutorial->title }}</p>
                        @if($tutorial->content)
                        <p class="text-xs text-gray-400 truncate max-w-xs mt-0.5">{{ Str::limit(strip_tags($tutorial->content), 80) }}</p>
                        @endif
                    </td>
                    ...
                    <td class="px-4 py-3 text-center">
                        <button wire:click="toggleActive({{ $tutorial->id }})"
                                class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium transition
                                       {{ $tutorial->is_active ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-gray-100 text-gray-500' }}">
                            {{ $tutorial->is_active ? 'Aktif' : 'Nonaktif' }}
                        </button>
                    </td>
                    <td class="px-4 py-3 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <button wire:click="openEdit({{ $tutorial->id }})"
                                    class="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </button>
                            <button wire:click="delete({{ $tutorial->id }})"
                                    wire:confirm="Hapus tutorial '{{ $tutorial->title }}'?"
    ...
    <div class="mt-4 text-xs text-gray-400 text-center">
        Link publik: <a href="{{ route('tutorial') }}" target="_blank" class="text-blue-600 hover:underline">{{ route('tutorial') }}</a>
    </div>
</div>
</div>
