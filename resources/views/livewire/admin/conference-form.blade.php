<div class="py-6 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
    ...
    {{-- TAB: General - Always keep in DOM for file uploads to work --}}
    <div class="bg-white rounded-xl shadow-sm border p-6 space-y-4" @if($activeTab !== 'general') style="display:none" @endif>
    ...
    {{-- TAB: Speakers - Always keep in DOM for file uploads --}}
    <div class="bg-white rounded-xl shadow-sm border p-6" @if($activeTab !== 'speakers') style="display:none" @endif>
        <div class="flex justify-between items-center mb-4">
        ...
        <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl shadow-sm border-2 border-blue-200 p-6 space-y-4">
            ...
            <div class="space-y-2">
                <div class="flex items-center justify-between">
                    <label class="block text-xs font-medium text-gray-600">Kontak Pembayaran</label>
                    <button type="button" wire:click="addPaymentContact" class="text-xs text-blue-600 hover:text-blue-800 flex items-center gap-1 font-medium">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>
                        Tambah Kontak
                    </button>
                </div>
                @forelse($paymentContacts as $index => $contact)
                <div class="flex items-center gap-2">
                    <div class="flex-1 relative">
                        <svg class="w-3.5 h-3.5 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                        <input wire:model="paymentContacts.{{ $index }}.name" type="text" class="w-full pl-9 pr-3 py-2 border rounded-lg text-sm bg-white focus:ring-blue-500 focus:border-blue-500" placeholder="Nama, cth: Budi Santoso">
                    </div>
                    <div class="flex-1 relative">
                        <svg class="w-3.5 h-3.5 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                        <input wire:model="paymentContacts.{{ $index }}.phone" type="text" class="w-full pl-9 pr-3 py-2 border rounded-lg text-sm bg-white focus:ring-blue-500 focus:border-blue-500" placeholder="No. HP, +62 812 xxxx xxxx">
                    </div>
                    <button type="button" wire:click="removePaymentContact({{ $index }})" wire:confirm="Hapus kontak ini?" class="text-red-500 hover:text-red-700 shrink-0 p-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                    </button>
                </div>
                @empty
                <p class="text-xs text-gray-400 italic">Belum ada kontak pembayaran. Klik "Tambah Kontak" untuk menambahkan.</p>
                @endforelse
            </div>
    ...
    {{-- TAB: Guidelines - Always keep in DOM for file uploads --}}
    <div class="bg-white rounded-xl shadow-sm border p-6 space-y-4" @if($activeTab !== 'guidelines') style="display:none" @endif>
    ...
    {{-- TAB: Template Luaran - Always keep in DOM for file uploads --}}
    <div class="bg-white rounded-xl shadow-sm border p-6 space-y-4" @if($activeTab !== 'templates') style="display:none" @endif>
        <div class="flex items-center justify-between">
            <div>
    ...
    {{-- TAB: Jurnal Publikasi - Always keep in DOM for file uploads --}}
    <div class="bg-white rounded-xl shadow-sm border p-6 space-y-4" @if($activeTab !== 'journals') style="display:none" @endif>
        <div class="flex items-center justify-between">
            <div>
