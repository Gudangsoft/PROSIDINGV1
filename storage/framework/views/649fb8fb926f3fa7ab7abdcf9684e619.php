    ...
    <div class="border-b border-gray-200 mb-6">
        <nav class="flex gap-6">
            <button wire:click="$set('activeTab', 'users')"
                    class="pb-3 text-sm font-medium border-b-2 transition <?php echo e($activeTab === 'users' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'); ?>">
            ...
            <button wire:click="$set('activeTab', 'roles')"
                    class="pb-3 text-sm font-medium border-b-2 transition <?php echo e($activeTab === 'roles' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'); ?>">
    ...
    <div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            ...
            <div class="flex items-center gap-2">
                <button wire:click="autoAssignAllRoles" wire:confirm="Otomatis assign role ke semua user yang belum memiliki role?"
                        class="inline-flex items-center gap-2 px-4 py-2 bg-amber-500 text-white rounded-lg text-sm font-medium hover:bg-amber-600 transition shrink-0 cursor-pointer">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    ...
                                    <img src="<?php echo e(asset('storage/' . $user->photo)); ?>" class="w-8 h-8 rounded-full object-cover shrink-0">
                                    <?php else: ?>
                            ...
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold
                                    ...
                                    <?php elseif($user->role === 'editor'): ?> bg-purple-100 text-purple-700
                                    <?php elseif($user->role === 'keuangan'): ?> bg-emerald-100 text-emerald-700
                                    <?php elseif($user->role === 'reviewer'): ?> bg-indigo-100 text-indigo-700
                                    <?php elseif($user->role === 'participant'): ?> bg-teal-100 text-teal-700
                                    <?php else: ?> bg-blue-100 text-blue-700
                                    <?php endif; ?>">
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($user->role === 'participant'): ?> Partisipan <?php elseif($user->role === 'keuangan'): ?> Keuangan <?php else: ?> <?php echo e(ucfirst($user->role)); ?> <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                </span>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-gray-600"><?php echo e($user->institution ?? '-'); ?></td>
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <form method="POST" action="<?php echo e(route('admin.impersonate', $user)); ?>" class="inline">
                                        <?php echo csrf_field(); ?>
                                        <button type="submit" title="Login As <?php echo e($user->name); ?>"
                                                class="p-1.5 rounded-lg text-yellow-600 hover:bg-yellow-50 transition cursor-pointer"
                                                onclick="return confirm('Login sebagai <?php echo e($user->name); ?>?')">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                        </button>
                                    </form>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                    <button wire:click="openUserDetail(<?php echo e($user->id); ?>)" title="Lihat Detail"
                                            class="p-1.5 rounded-lg text-gray-600 hover:bg-gray-100 transition cursor-pointer">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </button>
                                    <button wire:click="openAssignRole(<?php echo e($user->id); ?>)" title="Assign Role"
                                            class="p-1.5 rounded-lg text-green-600 hover:bg-green-50 transition cursor-pointer">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                                    </button>
                                    <button wire:click="openEditUser(<?php echo e($user->id); ?>)" title="Edit User"
                                    ...
                                    <button wire:click="confirmDelete('user', <?php echo e($user->id); ?>)" title="Hapus User"
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-5 py-4">
                                <div>
                                    <p class="font-medium text-blue-700"><?php echo e($role->name); ?></p>
                                    ...
                                    <p class="text-xs text-gray-400 mt-0.5"><?php echo e($role->description); ?></p>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                </div>
                            </td>
                            <td class="px-5 py-4">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    <?php if(in_array($role->permission_level, ['admin', 'manager'])): ?> bg-amber-100 text-amber-800
                                    <?php elseif($role->permission_level === 'editor'): ?> bg-purple-100 text-purple-800
                                    <?php elseif($role->permission_level === 'assistant'): ?> bg-teal-100 text-teal-800
                                    <?php elseif($role->permission_level === 'reviewer'): ?> bg-indigo-100 text-indigo-800
                                    <?php else: ?> bg-blue-100 text-blue-800
                                    <?php endif; ?>">
                                    <?php echo e($role->permission_level_label); ?>

                                </span>
                            </td>
                            
                            <td class="px-5 py-4 text-center">
                                <button wire:click="togglePermission(<?php echo e($role->id); ?>, 'submission')" class="cursor-pointer">
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($role->can_submission): ?>
                            ...
                            <td class="px-5 py-4 text-center">
                                <button wire:click="togglePermission(<?php echo e($role->id); ?>, 'review')" class="cursor-pointer">
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($role->can_review): ?>
                                    <svg class="w-5 h-5 mx-auto text-blue-600" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                                    <?php else: ?>
                                    <div class="w-5 h-5 mx-auto border-2 border-gray-300 rounded"></div>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            ...
                            <td class="px-5 py-4 text-center">
                                <button wire:click="togglePermission(<?php echo e($role->id); ?>, 'copyediting')" class="cursor-pointer">
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($role->can_copyediting): ?>
                                    <svg class="w-5 h-5 mx-auto text-blue-600" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                                    <?php else: ?>
                                    <div class="w-5 h-5 mx-auto border-2 border-gray-300 rounded"></div>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            ...
                            <td class="px-5 py-4 text-center">
                                <button wire:click="togglePermission(<?php echo e($role->id); ?>, 'production')" class="cursor-pointer">
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($role->can_production): ?>
                                    <svg class="w-5 h-5 mx-auto text-blue-600" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                                    <?php else: ?>
                                    <div class="w-5 h-5 mx-auto border-2 border-gray-300 rounded"></div>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-center gap-1">
                                    <button wire:click="openEditRole(<?php echo e($role->id); ?>)" title="Edit Role"
                                            class="p-1.5 rounded-lg text-blue-600 hover:bg-blue-50 transition cursor-pointer">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </button>
                                    <button wire:click="confirmDelete('role', <?php echo e($role->id); ?>)" title="Hapus Role"
                                            class="p-1.5 rounded-lg text-red-600 hover:bg-red-50 transition cursor-pointer">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                    </button>
                                </div>
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg relative z-10 max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-semibold text-gray-800">
                    <?php echo e($editingUser ? 'Edit User' : 'Tambah User Baru'); ?>
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                        Password <?php echo e($editingUser ? '(kosongkan jika tidak diubah)' : ''); ?> <span class="<?php echo e($editingUser ? '' : 'text-red-500'); ?>"><?php echo e($editingUser ? '' : '*'); ?></span>
                    </label>
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    ...
                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition cursor-pointer">
                        <?php echo e($editingUser ? 'Simpan Perubahan' : 'Buat User'); ?>
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($showAssignRoleModal): ?>
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md relative z-10">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-semibold text-gray-800">
                    Assign Roles — <?php echo e($assignUserName); ?>
            ...
            <div class="p-6">
                ...
                <div class="space-y-2 max-h-72 overflow-y-auto">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $allRoles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $role): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                    <label class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 hover:bg-gray-50 cursor-pointer transition">
                        <input type="checkbox" wire:model="assignRoles" value="<?php echo e($role->id); ?>"
                               class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                        <div class="flex-1">
                            <p class="text-sm font-medium text-gray-800"><?php echo e($role->name); ?></p>
                            <p class="text-xs text-gray-400"><?php echo e($role->permission_level_label); ?></p>
                        </div>
                        <div class="flex gap-1">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($role->can_submission): ?> <span class="w-2 h-2 rounded-full bg-green-400" title="Submission"></span> <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($role->can_review): ?> <span class="w-2 h-2 rounded-full bg-blue-400" title="Review"></span> <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($role->can_copyediting): ?> <span class="w-2 h-2 rounded-full bg-yellow-400" title="Copyediting"></span> <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($role->can_production): ?> <span class="w-2 h-2 rounded-full bg-purple-400" title="Production"></span> <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </div>
                    </label>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4" x-data x-init="$el.querySelector('input')?.focus()">
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg relative z-10 max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-semibold text-gray-800">
                    <?php echo e($editingRole ? 'Edit Role' : 'Buat Role Baru'); ?>
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                ...
                <div>
                    ...
                    <select wire:model="rolePermissionLevel"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $permissionLevels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" wire:click="closeModal" class="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition cursor-pointer">Batal</button>
                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition cursor-pointer">
                        <?php echo e($editingRole ? 'Simpan Perubahan' : 'Buat Role'); ?>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm relative z-10">
            <div class="p-6 text-center">
                ...
                <h3 class="text-lg font-semibold text-gray-800 mb-2">Hapus <?php echo e($deleteType === 'user' ? 'User' : 'Role'); ?>?</h3>
                <p class="text-sm text-gray-500 mb-6">
                    Anda yakin ingin menghapus <strong><?php echo e($deleteName); ?></strong>?
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($deleteType === 'role'): ?> Semua user yang memiliki role ini akan kehilangan akses tersebut. <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($showUserDetailModal && $detailUser): ?>
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl relative z-10 max-h-[90vh] overflow-y-auto">
            ...
            <div class="p-6">
                ...
                <div class="flex items-center gap-4 mb-6">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($detailUser->photo): ?>
                    <img src="<?php echo e(asset('storage/' . $detailUser->photo)); ?>" class="w-16 h-16 rounded-full object-cover border-2 border-gray-200">
                    <?php else: ?>
                    <div class="w-16 h-16 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-xl">
                        <?php echo e(strtoupper(substr($detailUser->name, 0, 1))); ?>

                    </div>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    <div>
                        <h4 class="text-xl font-bold text-gray-800"><?php echo e($detailUser->name); ?></h4>
                        <p class="text-sm text-gray-500"><?php echo e($detailUser->email); ?></p>
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold mt-1
                            <?php if($detailUser->role === 'admin'): ?> bg-red-100 text-red-700
                            <?php elseif($detailUser->role === 'editor'): ?> bg-purple-100 text-purple-700
                            <?php elseif($detailUser->role === 'reviewer'): ?> bg-indigo-100 text-indigo-700
                            <?php elseif($detailUser->role === 'participant'): ?> bg-teal-100 text-teal-700
                            <?php else: ?> bg-blue-100 text-blue-700
                            <?php endif; ?>">
                            <?php echo e($detailUser->role === 'participant' ? 'Partisipan' : ucfirst($detailUser->role)); ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div class="bg-gray-50 rounded-lg p-3">
                        ...
                        <p class="text-gray-800"><?php echo e($detailUser->gender === 'male' ? 'Laki-laki' : ($detailUser->gender === 'female' ? 'Perempuan' : '-')); ?></p>
                    </div>
                    <div class="bg-gray-50 rounded-lg p-3">
                        <p class="text-xs font-medium text-gray-500 uppercase mb-1">Telepon</p>
                        <p class="text-gray-800"><?php echo e($detailUser->phone ?? '-'); ?></p>
                    </div>
                    <div class="bg-gray-50 rounded-lg p-3">
                        <p class="text-xs font-medium text-gray-500 uppercase mb-1">Institusi</p>
                        <p class="text-gray-800"><?php echo e($detailUser->institution ?? '-'); ?></p>
                    </div>
                    <div class="bg-gray-50 rounded-lg p-3">
                        <p class="text-xs font-medium text-gray-500 uppercase mb-1">Negara</p>
                        <p class="text-gray-800"><?php echo e($detailUser->country ?? '-'); ?></p>
                    </div>
                    <div class="bg-gray-50 rounded-lg p-3 md:col-span-2">
                        ...
                        <p class="text-gray-800"><?php echo e($detailUser->research_interest ?? '-'); ?></p>
                    </div>
                    <div class="bg-gray-50 rounded-lg p-3 md:col-span-2">
                        <p class="text-xs font-medium text-gray-500 uppercase mb-1">Informasi Lainnya</p>
                        <p class="text-gray-800"><?php echo e($detailUser->other_info ?? '-'); ?></p>
                    </div>
                    <div class="bg-gray-50 rounded-lg p-3">
                        <p class="text-xs font-medium text-gray-500 uppercase mb-1">Assigned Roles</p>
                        <div class="flex flex-wrap gap-1 mt-1">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $detailUser->roles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $role): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-200 text-gray-700"><?php echo e($role->name); ?></span>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
                    ...
                    <div class="bg-gray-50 rounded-lg p-3">
                        ...
                        <p class="text-gray-800"><?php echo e($detailUser->created_at->format('d M Y, h:i A')); ?></p>
                    </div>
                </div>

                
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($detailUser->signature || $detailUser->proof_of_payment): ?>
                <div class="mt-4 pt-4 border-t border-gray-100">
                    <p class="text-xs font-medium text-gray-500 uppercase mb-3">File Upload</p>
                    <div class="flex flex-wrap gap-4">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($detailUser->proof_of_payment): ?>
                        <div>
                            <p class="text-xs text-gray-600 mb-1">Bukti Pembayaran</p>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(str_ends_with($detailUser->proof_of_payment, '.pdf')): ?>
                            <a href="<?php echo e(asset('storage/' . $detailUser->proof_of_payment)); ?>" target="_blank" class="text-sm text-blue-600 hover:text-blue-800 font-medium">Lihat PDF</a>
                            <?php else: ?>
                            <img src="<?php echo e(asset('storage/' . $detailUser->proof_of_payment)); ?>" class="max-h-24 rounded border cursor-pointer" onclick="window.open(this.src)">
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </div>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($detailUser->signature): ?>
                        <div>
                            <p class="text-xs text-gray-600 mb-1">Tanda Tangan</p>
                            <img src="<?php echo e(asset('storage/' . $detailUser->signature)); ?>" class="max-h-24 rounded border">
                        </div>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-4 mt-4 border-t border-gray-100">
                    ...
                    <button wire:click="closeUserDetail" wire:click.prevent="openEditUser(<?php echo e($detailUser->id); ?>)" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition cursor-pointer">Edit User</button>
                </div>
            </div>
        </div>
    </div>
...
<?php /**PATH D:\LPKD-APJI\PROSIDINGV1\resources\views\livewire\admin\users-roles.blade.php ENDPATH**/ ?>