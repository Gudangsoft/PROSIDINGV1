...
|--------------------------------------------------------------------------
|
| The entire Prosiding application (registration, papers, payments, admin,
| reviewer, etc.) lives here. These routes only resolve on a domain that
| belongs to a tenant (InitializeTenancyByDomain) and are explicitly
| blocked on central_domains (PreventAccessFromCentralDomains) so the
| central SaaS site and a tenant's conference site can never collide on
| the same URI.
|
*/
